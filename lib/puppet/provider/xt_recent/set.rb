Puppet::Type.type(:xt_recent).provide(:set) do
  ...
  def initialize(*args)
    ...
    Dir.glob("#{resource[:name]}/*").each do |file|
      ...
      File.chmod(0o600, file) unless File.stat(file).mode.anybits?(0o200)
    end
  end
  ...
  def ip_list_hash_size
    ...
    # check it.
    return resource[__method__] if resource[__method__].to_s == '0'
  ...
  def ip_list_perms
    ...
    '%#o' % File.read("#{resource[:name]}/#{__method__}").chomp.to_i
  end
