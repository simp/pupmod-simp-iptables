...
class PuppetX::SIMP::IPTables
  ...
  def slice_array(to_slice, max_length)
    ...
    cluster_locations = to_slice.map { |x| x.to_s.include?(':') }
    ...
    retval = (0...num_groups).map do |x|
      to_slice[(x * max_length)..((x * max_length) + max_length - 1)].compact
    end

    i = 0
    retval.each do |arr|
      arr.each_with_index do |_x, j|
