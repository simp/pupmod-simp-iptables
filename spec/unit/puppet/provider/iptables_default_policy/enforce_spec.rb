...
describe provider_class do
  ...
  before(:each) do
    ...
    allow(FileTest).to receive(:file?).with(File.join(first_path_dir, 'iptables')).and_return true
    allow(FileTest).to receive(:exist?).with(File.join(first_path_dir, 'iptables')).and_return true
    allow(FileTest).to receive(:executable?).with(File.join(first_path_dir, 'iptables')).and_return true

    allow(FileTest).to receive(:file?).with(File.join(first_path_dir, 'iptables-save')).and_return true
    allow(FileTest).to receive(:exist?).with(File.join(first_path_dir, 'iptables-save')).and_return true
    allow(FileTest).to receive(:executable?).with(File.join(first_path_dir, 'iptables-save')).and_return true

    allow(FileTest).to receive(:file?).with(File.join(first_path_dir, 'ip6tables')).and_return true
    allow(FileTest).to receive(:exist?).with(File.join(first_path_dir, 'ip6tables')).and_return true
    allow(FileTest).to receive(:executable?).with(File.join(first_path_dir, 'ip6tables')).and_return true

    allow(FileTest).to receive(:file?).with(File.join(first_path_dir, 'ip6tables-save')).and_return true
  ...
  context '#flush' do
    context 'ipv4' do
      it 'updates the filter:INPUT chain for ipv4' do
        table = 'filter'
        chain = 'INPUT'
        policy = 'DROP'
    ...
    context 'ipv6' do
      it 'updates the filter:INPUT chain for ipv6' do
        table = 'filter'
        chain = 'INPUT'
        policy = 'DROP'
    ...
    context 'both' do
      it 'updates the filter:INPUT chain for ipv4 and ipv6' do
        table = 'filter'
        chain = 'INPUT'
        policy = 'DROP'
