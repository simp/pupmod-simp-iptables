...
describe 'iptables::slice_ports' do
  context 'when single port' do
    ...
    it 'returns a single array with input' do
  ...
  context 'when array of single ports' do
    ...
    context 'when max_length is < input array size' do
      it 'returns multiple sub-arrays' do
  ...
  context 'when single range' do
    ...
    it 'returns one sub-array if max_length >= 2' do
  ...
  context 'when array of ranges' do
    ...
    it 'treats each range as 2 entries' do
    ...
    it 'does not split any range into sub-arrays' do
      expected = [['10:20'], ['110:120'], ['210:220']]
  ...
  context 'when mixed array of individual ports and ranges ' do
    ...
    it 'treats each range a 2 entries' do
  ...
  context 'failures' do
    ...
    it 'fails if max_length is 1 and input contains a port range' do
      # Where this function is used, max_length = 15, so not going
      # to worry about fixing this edge case for now.
      is_expected.to run.with_params(input, 1).and_raise_error(%r{max_length must be >=2 when input has a port range})
    end
  end
end
